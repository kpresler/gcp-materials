...
# the for loop definitely seems easier!
...
# talk more about lists in a bit
...
# this will become more obvious when we talk about lists later on, because
...
# in elementary school, one of the things you probably learned was your "times tables"
# in case you've forgotten them, we can use Python to help us recreate them

# start range at 1 ... anything * 0 is not very interesting
for i in range(1, 11):
    for j in range(1, 11):
        print(f"{i:<2} x {j:>2} = {i*j:>3}")
        
# We'll practice with something a bit more interesting in a bit